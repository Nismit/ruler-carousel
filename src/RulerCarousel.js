...
export class RulerCarousel {
   ...
   * @param {HTMLElement} [options.eventTarget] - Element to attach events to
   * @param {Array<{name: string, type: string, label: string}>} [options.items] - Carousel items
   * @param {number} [options.initialIndex] - Initial selected index
   * @param {(index: number, item: any) => void} [options.onSelect] - Selection callback
   * @param {(index: number, item: any) => void} [options.onConfirm] - Confirm (double-tap) callback
   * @param {() => void} [options.onBackgroundTap] - Background tap callback
   */
