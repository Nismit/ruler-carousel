...
/**
 * 3D pad control for vec3 values.
 * Double-tap or click mode label to toggle between XY and Z modes.
 *
 * @typedef {{x: number, y: number, z: number}} Vec3
 */
export class RulerVec3Control extends RulerBaseControl {
  static DOUBLE_TAP_DELAY = 300;
  static TAP_MOVE_THRESHOLD = 10;
  static Z_SENSITIVITY = 0.005;
  ...
  /** @type {((mode: 'xy'|'z') => void)|null} */
  onModeChange = null;

  /**
   * @param {{x: number, y: number, z: number}} [initialValue]
   */
  constructor(initialValue = { x: 0, y: 0, z: 0 }) {
    super(initialValue);
  }
  ...
  _createDOM() {
    ...
    this.element.className = 'ruler-pad ruler-pad--vec3';
  ...
  _setMode(mode) {
    this._mode = mode;
    if (mode === 'z') {
      this._thumb.classList.add('z-mode');
      this._modeLabel.classList.add('z-mode');
      this._modeLabel.textContent = 'Z';
    } else {
      this._thumb.classList.remove('z-mode');
      this._modeLabel.classList.remove('z-mode');
   ...
   */
  _toggleMode() {
    this._setMode(this._mode === 'xy' ? 'z' : 'xy');
  }
  ...
  _onMouseMove(e) {
    ...
    if (dist > RulerVec3Control.TAP_MOVE_THRESHOLD) {
      this._hasMoved = true;
    }
    ...
    if (this._hasMoved) {
      if (this._mode === 'xy') {
        ...
        this._updateZFromDelta(dx);
        this._touchStartPos.x = e.clientX;
      }
    }
  }
  ...
  _onTouchMove(e) {
    ...
    if (this._hasMoved) {
      if (this._mode === 'xy') {
        ...
        this._updateZFromDelta(dx);
        this._touchStartPos.x = e.touches[0].clientX;
      }
    }
  }
  ...
  _onTouchEnd(e) {
    if (e.touches.length === 0) {
      ...
      if (!this._hasMoved) {
        const now = Date.now();
        if (now - this._lastTapTime < RulerVec3Control.DOUBLE_TAP_DELAY) {
          this._toggleMode();
          this._lastTapTime = 0;
        } else {
   ...
   * @param {number} deltaX
   */
  _updateZFromDelta(deltaX) {
    const rangeZ = this._max.z - this._min.z;
    const deltaZ = deltaX * RulerVec3Control.Z_SENSITIVITY * rangeZ;
    const newZ = Math.max(this._min.z, Math.min(this._max.z, this._value.z + deltaZ));

    this._value = { ...this._value, z: newZ };
    this._updateUI();
    this._emitChange();
  }
   ...
   */
  _updateRange() {
    this._min = { x: -this._scale, y: -this._scale, z: -this._scale };
    this._max = { x: this._scale, y: this._scale, z: this._scale };
  }
